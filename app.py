...
def load_prices_from_db() -> pd.DataFrame:
    ...
    conn.close()
    if df.empty:
        df = pd.DataFrame(columns=["EAN","Referentie","Verkoopprijs","Inkoopprijs","Verzendkosten","Overige kosten","Leverancier","MOQ","Levertijd (dagen)"])
    # type coercion
...
if "prices_df" not in st.session_state:
    ...
    try:
        ...
        st.session_state.prices_df = pd.DataFrame(columns=["EAN","Referentie","Verkoopprijs","Inkoopprijs","Verzendkosten","Overige kosten","Leverancier","MOQ","Levertijd (dagen)"])
...
def merged_frame():
    ...
    # merge prijzen
    cols_prices = [c for c in ["Referentie","Verkoopprijs","Inkoopprijs","Verzendkosten","Overige kosten","Leverancier","MOQ","Levertijd (dagen)"] if c in prices.columns]
    if cols_prices:
...
with T2:
    ...
    if data is None:
        ...
        display_cols = [
            "Status","EAN","Referentie","Titel","Vrije voorraad","Incoming","Verkoopprognose min (Totaal 4w)",
...
with T3:
    ...
    if data is None:
        ...
        if df_order.empty:
            ...
            st.dataframe(df_order[["Leverancier","EAN","Referentie","Titel","Aanbevolen bestelaantal","Totale kostprijs per stuk","Totaal kosten"]], use_container_width=True)
